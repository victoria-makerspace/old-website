-- Storage values
	...
	SELECT generate_series(1,12), 'storage-locker-hallway';
	-- Bathroom lockers
	INSERT INTO storage (number, plan_id)
	SELECT generate_series(1,11), 'storage-locker-bathroom';
		-- Bathroom lockers 7 and 8 are reserved for VITP cleaners
		UPDATE storage
		SET available = false
		WHERE number IN (7, 8)
			AND plan_id = 'storage-locker-bathroom';
	-- Wall storage
	INSERT INTO storage (number, plan_id, quantity)
	SELECT generate_subscripts(a, 1), 'storage-wall-lineal-foot', unnest(a)
	FROM (
		...
		SET available = false
		WHERE number IN (1, 2)
			AND plan_id = 'storage-wall-lineal-foot';
